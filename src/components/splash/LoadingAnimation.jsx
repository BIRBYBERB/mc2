    ...
    <div className={`w-full max-w-xs sm:max-w-sm transition-all duration-1000 ${
      ...
      <div className="relative mb-6 sm:mb-8">
        <div className="w-full h-3 sm:h-4 bg-white/30 backdrop-blur-sm border border-white/40 rounded-xl sm:rounded-2xl overflow-hidden shadow-inner">
          <div
            className="h-full bg-gradient-to-r from-green-600 via-green-500 to-green-400 rounded-xl sm:rounded-2xl transition-all duration-300 ease-out shadow-sm relative"
        ...
        <div className="flex justify-between items-center mt-3 sm:mt-4">
          ...
          <div className="flex items-center space-x-1.5 sm:space-x-2">
            <span className="text-green-600 text-xs xs:text-sm font-bold">{Math.min(progress, 100)}%</span>
            <div className={`w-1.5 sm:w-2 h-1.5 sm:h-2 bg-green-500 rounded-full ${
      ...
      <div className="flex justify-center space-x-2 sm:space-x-3">
        <div className={`w-2.5 sm:w-3 h-2.5 sm:h-3 rounded-full shadow-md sm:shadow-lg transition-all duration-500 ${
          progress < 100 ? 'bg-green-700 animate-bounce' : 'bg-green-400 animate-pulse'
        }`} style={{ animationDuration: '1s' }} />
        <div className={`w-2.5 sm:w-3 h-2.5 sm:h-3 rounded-full shadow-md sm:shadow-lg transition-all duration-500 ${
          progress < 100 ? 'bg-green-600 animate-bounce' : 'bg-green-500 animate-pulse'
        }`} style={{ animationDelay: '200ms', animationDuration: '1s' }} />
        <div className={`w-2.5 sm:w-3 h-2.5 sm:h-3 rounded-full shadow-md sm:shadow-lg transition-all duration-500 ${
          progress < 100 ? 'bg-green-500 animate-bounce' : 'bg-green-600 animate-pulse'
        }`} style={{ animationDelay: '400ms', animationDuration: '1s' }} />
        <div className={`w-2 sm:w-2 h-2 sm:h-2 rounded-full shadow-sm sm:shadow-md opacity-75 transition-all duration-500 ${
          progress < 100 ? 'bg-green-400 animate-bounce' : 'bg-green-500 animate-pulse'
        ...
        <div className={`w-2 sm:w-2 h-2 sm:h-2 rounded-full shadow-sm sm:shadow-md opacity-50 transition-all duration-500 ${
          progress < 100 ? 'bg-green-300 animate-bounce' : 'bg-green-600 animate-pulse'
