...
export default function DesktopNavbar({ currentPage, onNavigate }) {
  const navItems = [
    { id: 'home', label: 'Home' },
    { id: 'makanan', label: 'Buildings' },
    { id: 'minuman', label: 'Mob Farms' },
    { id: 'profile', label: 'Profile' }
  ];

  return (
    <nav className="hidden md:block shadow-lg border-b border-green-100 sticky top-0 z-50 backdrop-blur-sm bg-white/95">
        ...
        <div className="flex justify-between items-center h-20">
          ...
          <div className="flex items-center space-x-4">
            <div className="relative group">
              ...
              <div className="absolute -bottom-0.5 -left-0.5 w-1 h-1 bg-green-300 rounded-full animate-ping opacity-50" style={{ animationDelay: '300ms' }} />
            </div>
            <div>
              <h1 className="text-2xl font-bold bg-gradient-to-r from-slate-800 via-slate-700 to-slate-800 bg-clip-text text-transparent">
                Schrodinger's
              </h1>
              <h2 className="text-base font-semibold bg-gradient-to-r from-green-600 via-green-500 to-green-400 bg-clip-text text-transparent -mt-1">
                Buildings
              </h2>
            </div>
          </div>
              ...
              <button
                ...
                className={`px-4 py-3 text-base font-medium transition-all duration-200 border-b-2 ${
                  currentPage === item.id
                    ...
                    : 'text-slate-600 border-transparent hover:text-green-500 hover:border-green-300'
                }`}
              >
                {item.label}
              </button>
